...
def search_keyword(keyword,dir,date)
	...
	if keyword  != nil && dir != nil
		...
		Dir.chdir("work")
		FileUtils::mkdir_p("#{dir}_no_#{keyword}/sum") unless FileTest.exist?("#{dir}_no_#{keyword}/sum")
		Dir.glob("#{dir}/#{date}*.csv").each {|all_csv_file|
			...
			puts "sum:#{csv_length} Hit:#{k} No:#{i} #{File.basename(all_csv_file)}"
			sum_ary[sum_num] = "#{k},#{i},#{csv_length},#{File.basename(all_csv_file)}\n"
